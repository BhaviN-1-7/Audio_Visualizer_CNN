...
class AudioProcessor:
    def __init__(self):
        self.transform = nn.Sequential(
            T.MelSpectrogram(
                sample_rate=22050,##Higher audio quality (44100 Hz) but lower frequency resolution (22050 Hz MelSpectrogram)
                ##This creates a specific "compressed" frequency representation that your model learned
                ##When you used 44100 Hz for MelSpectrogram, it created a different frequency representation that the model didn't recognize
                n_fft=1024,
                hop_length=512,
                n_mels=128,
                f_min=0,
    ...
    def process_audio_chunk(self, audio_data):
        ...
        spectrogram = self.transform(waveform)#waveform to spectrogram
...
class AudioClassifier:
    ...
    ##########
            #output=self.modal(spectrogram)
            # output = self.model(spectrogram)
            output = torch.nan_to_num(output)
            probabilities = torch.softmax(output, dim=1)
            #dim 0 is class and dim 1 is (batch_size,num_class)
